...
*   **食材細分分類與預估保存期限 (Shelf Life Estimation)**：
    *   系統須能針對不同屬性的食材提供預設的保存天數。特別針對「蔬菜」類別，需細分為「葉菜類」與「根莖類/其他蔬菜」，並給予不同的預設保存期限。
    *   **預設保存期限對照表 (基於購買日天數)**：
        *   `蔬菜類 - 葉菜類` (如：高麗菜、青江菜、菠菜、空心菜、萵苣)：預設 **4 天**。
        *   `蔬菜類 - 根莖類/其他` (如：胡蘿蔔、馬鈴薯、洋蔥、大蒜、薑)：預設 **14 天**。
        *   `肉類`：冷藏預設 **3 天**，冷凍預設 **90 天**（入庫時需指定或由系統判定）。
        *   `海鮮類`：冷藏預設 **2 天**，冷凍預設 **30 天**。
        *   `水果類`：預設 **7 天**。
        *   `乳製品`：預設 **7 天**。
        *   `冷凍食品/火鍋料`：預設 **180 天**。
        *   `調味料`：預設 **365 天**。
        *   `其他`：預設 **7 天**。
    *   *主動填充機制*：當使用者登錄食材且未輸入「到期日」時，系統應根據「購買日期 + 預設保存天數」自動計算並填寫預計到期日。
*   **細分類別判定機制 (AI & 本地正則)**：
    *   *AI 自動識別*：調用 Gemini API 時，Prompt 需設計為能準確判斷食材是屬於「葉菜類」、「根莖類/其他」或是其他類別。
    *   *本地正則備援*：在無 API 金鑰或網路中斷時，使用本機正則表達式庫進行備援分類：
        *   `葉菜類` 關鍵字匹配：`菜`、`菠`、`萵`、`茼`、`芹`、`莧`。
        *   `根莖類` 關鍵字匹配：`蘿蔔`、`薯`、`芋`、`洋蔥`、`蒜`、`薑`、`筍`、`淮山`、`山藥`。
*   **過期損耗與折舊診斷 (Financial Loss Diagnosis)**：
    *   為了計算食材過期造成的金錢浪費，食材 Schema 與資料庫中需額外支援 **「單價/價格 (price)」** 欄位（單位為元，預設為 0）。
    *   **效期狀態與損耗評估算法**：
        *   🔴 **已過期 (Expired)**：當前系統日期大於食材到期日。損耗金額 = `剩餘數量 * 單價`（損失 100% 價值）。
        *   🟡 **即將過期 (Expiring Soon)**：當前系統日期距離到期日小於或等於 $N$ 天（預設為 3 天）。潛在折舊/損失金額 = `剩餘數量 * 單價`，並特別警示使用者優先烹飪，以避免浪費。
        *   🟢 **新鮮 (Fresh)**：當前系統日期距離到期日大於 $N$ 天。折舊/損耗金額 = 0。
